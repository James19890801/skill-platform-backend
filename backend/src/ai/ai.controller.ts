...
export class AiController {
  ...
  @Post('plan-skills')
  @ApiOperation({ summary: 'AI 规划 Skill' })
  @ApiBody({ type: PlanSkillsDto })
  @ApiResponse({ status: 200, type: PlanSkillsResponse })
  ...
  async planSkills(@Body() body: PlanSkillsInput): Promise<PlanSkillsResponse> {
    try {
      ...
      throw new HttpException({ success: false, data: [], message: error instanceof Error ? error.message : 'AI 服务调用失败' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  @Post('chat')
  @ApiOperation({ summary: 'AI 对话（流式输出）' })
  ...
  async chat(@Body() body: ChatDto, @Res() res: Response) {
    try {
      ...
      throw new HttpException({ success: false, message: error instanceof Error ? error.message : 'AI 对话失败' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async exportDocx(@Body() body: ExportDto, @Res() res: Response) {
    try {
      ...
      throw new HttpException({ success: false, message: error instanceof Error ? error.message : '导出失败' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  clearConversation(@Param('threadId') threadId: string) {
    ...
    if (!deleted) {
      throw new HttpException({ success: false, message: '会话不存在' }, HttpStatus.NOT_FOUND);
    }
    return { success: true, message: '会话已清除' };
  }

  @Get('preview/:token')
  @ApiOperation({ summary: '预览工具生成的文档' })
  async previewFile(@Param('token') token: string, @Res() res: Response) {
    const preview = await this.aiService.generatePreview(token);
    if (!preview) {
      throw new HttpException({ success: false, message: '文件不存在或已过期，或预览生成失败' }, HttpStatus.NOT_FOUND);
    }
  ...
  async viewReport(@Param('token') token: string, @Res() res: Response) {
    ...
    if (!report) {
      throw new HttpException({ success: false, message: '报告不存在或已过期' }, HttpStatus.NOT_FOUND);
    }
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
  ...
  async executeSkill(@Param('id') id: string, @Body() body: { input: string; threadId?: string }) {
    try {
      const result = await this.skillExecutor.execute(Number(id), body.input || '', body.threadId);
      return { success: true, data: result, message: `Skill 执行完成，共 ${result.totalRounds} 轮，产出 ${result.artifacts.length} 个产物` };
    } catch (error) {
      if (error instanceof NotFoundException) {
        throw new HttpException({ success: false, message: error.message }, HttpStatus.NOT_FOUND);
      }
      throw new HttpException({ success: false, message: error instanceof Error ? error.message : 'Skill 执行失败' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  @Get('execute-skill/:id/history')
  @ApiOperation({ summary: '获取 Skill 执行记录' })
  async getExecutionHistory(@Param('id') id: string) {
    try {
      const history = await this.skillExecutor.getHistory(Number(id));
      return { success: true, data: history };
    } catch (error) {
      throw new HttpException({ success: false, message: error instanceof Error ? error.message : '查询失败' }, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  @Get('execute-skill/execution/:executionId')
  @ApiOperation({ summary: '获取单次执行详情' })
  async getExecutionDetail(@Param('executionId') executionId: string) {
    try {
      ...
      if (!detail) {
        throw new HttpException({ success: false, message: '执行记录不存在' }, HttpStatus.NOT_FOUND);
      }
