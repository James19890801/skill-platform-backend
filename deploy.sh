...
push_to_github() {
    ...
    if [ -n "$(git status --porcelain)" ]; then
        ...
        if git diff --cached --quiet; then
            print_warning "只有运行时数据库/备份变化，跳过代码提交"
        else
...
# 发布前保护本地 SQLite 数据库快照
backup_local_database() {
    local db_path="/Users/Administrator/Desktop/skill-platform/backend/database.sqlite"
    local backup_dir="/Users/Administrator/Desktop/skill-platform/backups"

    if [ -f "$db_path" ]; then
        mkdir -p "$backup_dir"
        local backup_path="$backup_dir/database.$(date +%Y%m%d-%H%M%S).sqlite"
        cp "$db_path" "$backup_path"
        print_success "本地数据库已备份: $backup_path"
    else
        print_warning "未找到本地 SQLite 数据库，跳过本地备份"
    fi
}
